...
# ----------------------------------------------------
    ...
    n_top = len(top_skus)
    st.iframe(html_top_wrapped, height=68 + n_top * 60)
    ...
    if dead_stock_list:
        ...
        st.iframe(html_dead_wrapped, height=80 + len(dead_stock_list) * 58)
...
# ----------------------------------------------------
    ...
    if scorecard_list:
        ...
        st.iframe(html_sc_wrapped, height=80 + len(scorecard_list) * 58)
    ...
    if better_alts:
        ...
        st.iframe(html_alts_wrapped, height=80 + len(better_alts) * 58)
    else:
    ...
    with tab_orders:
        ...
        if exec_report:
            cost_match = re.search(
                r"(?:Total Proposed Order Value|Current Order Value|Proposed Order Value):\s*\$?\s*([\d,]+\.?\d*)",
                exec_report)
            if cost_match:
                proposed_cost = float(cost_match.group(1).replace(",", ""))
        ...
        status_fg = "#10B981" if is_compliant else "#EF4444"
        ...
        approved_count = sum(
            1 for item in recs if st.session_state.get(
                f"approved_{item['sku']}", False))
        rejected_count = sum(
            1 for item in recs if st.session_state.get(
                f"rejected_{item['sku']}", False))
            ...
            if total_actioned > 0:
                ...
                with col_reset:
                    if st.button(
                        ...
                        keys_to_delete = [
                            k for k in st.session_state if k.startswith(
                                ("approved_", "rejected_"))]
                ...
                st.info(
                    "No recommendations available. Run the AI agent to generate proposals.")
        else:
        ...
        for idx, item in enumerate(recs):
            ...
            with col_action_btns:
                ...
                if is_approved:
                    ...
                    """
                st.html(btn_html)
>>>>>>> ebf7c5a1ced4fc0d7535d8b94b9258b0f56b6838
        ...
        if plan_list:
            ...
            st.iframe(html_plan_wrapped, height=80 + len(plan_list) * 58)

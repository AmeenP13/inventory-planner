...
def send_desktop_notification(title: str, message: str) -> None:
    ...
    if DESKTOP_NOTIFICATIONS_AVAILABLE:
        try:
            ...
            print(
                f"(Desktop notification failed: {e}. Showing console alert instead.)")
...
def run_check(
    ...
    current_alerts = report[report["stock_status"].isin(
    ...
    newly_alerting = current_alerts[current_alerts["product_id"].isin(
        newly_alerting_ids)]
    ...
    if not newly_alerting.empty:
        ...
        more = "" if len(
            newly_alerting) <= 8 else f" (+{len(newly_alerting) - 8} more)"
        message = f"{
            len(newly_alerting)} product(s) just dropped to low/out of stock: {names}{more}"
    ...
    # Products that recovered (left alert status) — useful to know, not
    # alerted on
    recovered_ids = previous_alert_ids - current_alert_ids
    if recovered_ids:
        print(
            f"{len(recovered_ids)} product(s) recovered above reorder point since last check.")
...
def main():
    parser = argparse.ArgumentParser(
        description="Live Inventory Monitor with change-triggered alerts")
    parser.add_argument(
        "--csv",
        required=True,
        help="Path to the CSV file to watch")
    parser.add_argument(
        "--service-level",
        type=float,
        default=0.95,
